...
if __name__ == '__main__':
    ...
    old_spm_tokens_set = {p.piece for p in old_spm.pieces}
    ...
    ## Save
    output_sp_dir = f'{output_dir}/merged_tokenizer_sp'
    output_hf_dir = f'{output_dir}/merged_tokenizer_hf'
    os.makedirs(output_sp_dir,exist_ok=True)
    with open(f'{output_sp_dir}/merged_tokenizer.model', 'wb') as f:
        f.write(old_spm.SerializeToString())

    try:
        tokenizer = AutoTokenizer.from_pretrained(
            pretrained_model_name_or_path=tokenizer_dir,
            vocab_file=f'{output_sp_dir}/merged_tokenizer.model',
            use_fast=False,
        )
    except RecursionError:
        tokenizer = AutoTokenizer.from_pretrained(
    ...
    logging.info("Merged tokenizer has been saved to %s", output_dir)


    # Test
    new_tokenizer = tokenizer
    logging.info("Old tokenizer vocab size: %d", len(old_tokenizer))
    logging.info("New tokenizer vocab size: %d", len(new_tokenizer))

    text='''白日依山尽，黄河入海流。欲穷千里目，更上一层楼。
    The primary use of LLaMA is research on large language models, including'''
    logging.info(f"Test text:\n %s",text)
    logging.info(
        "Tokenized by original tokenizer:%s", old_tokenizer.tokenize(text)
    )
    logging.info("Tokenized by merged tokenizer:%s", new_tokenizer.tokenize(text))